...
bool Manager::validateSpecialCode(const char* specialCode) const
{
	...
	char* idWithSuffix = StrOpr::concatChar(idStr, FileName::SPECIAL_CODE_SUFFIX);
	char* fileName = StrOpr::concatChar(FileName::SECURE_DIR, idWithSuffix);
	delete[] idStr;
	delete[] idWithSuffix;
...
bool Manager::promoteCashier(Worker** allWorkers, size_t workersCount, size_t cashierIdToPromote, const char* specialCode) const
{
	...
	char* newManagerFileName = StrOpr::concatChar(newManagerIdStr, FileName::SPECIAL_CODE_SUFFIX);
